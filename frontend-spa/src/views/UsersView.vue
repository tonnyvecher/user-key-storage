<template>
  <div class="card">
    ...
    <p class="hint">
      Здесь отображаются пользователи из внутренней БД (<code>users</code>), а
      также агрегированный статус целостности их ролей (проверка через
      криптографические подписи и ключи из Vault).
    </p>
    ...
    <table v-if="filteredUsers.length > 0" class="table">
      ...
      <tbody>
        <tr
          ...
          :key="u.id"
          :class="{ 'row--me': myUserId === u.id }"
        >
          <td>
            {{ u.primary_email }}
            <span v-if="myUserId === u.id" class="you-label">(Вы)</span>
          </td>
          <td>
          ...
          <td>{{ u.roles_count }}</td>
          <td>
            <span :class="['pill', u.has_issues ? 'pill--bad' : 'pill--ok']">
...
const issuesCount = computed(
  () => users.value.filter((u) => u.has_issues).length
);
...
.row--me {
  background: #ecfdf5; /* лёгкий зелёный фон */
}
