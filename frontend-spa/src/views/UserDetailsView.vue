<template>
    <div>
      <div class="card">
        <h2 class="card__title">Пользователь {{ shortId }}</h2>
        <p class="hint">
          Это личный кабинет пользователя. Здесь показываются его профиль, роли и результат проверки доступа через
          ...
          <div v-if="rolesData && rolesData.roles.length > 0">
            ...
            >
              ...
              <div class="role-row__meta">
                ...
                <span v-if="r.integrity">
                  Целостность: {{ r.integrity }}
                </span>
              </div>
