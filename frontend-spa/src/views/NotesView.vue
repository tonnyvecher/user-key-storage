<template>
  <div class="card">
    ...
    <p class="hint">
      Здесь можно создавать заметки с минимальной ролью доступа (USER / MANAGER
      / ADMIN / SUPERADMIN / OWNER). Доступ к заметкам определяется максимальной
      валидной ролью пользователя в нашей системе. Редактировать заметку может
      только её владелец и только в пределах своей максимальной роли.
    </p>
    ...
    <section class="section">
      ...
      <p class="hint">
        Пользователь может создавать заметки только с <strong>min_role</strong>,
        не превышающей его максимальную роль.
      </p>
      ...
      <div class="field">
        ...
        <select id="minRole" v-model="newMinRole">
          ...
          <option v-for="r in availableMinRoles" :key="r" :value="r">
            {{ r }}
          </option>
        </select>
      </div>
      ...
      <div v-if="noteError" class="error">
        Ошибка при создании заметки: {{ noteError }}
      </div>
    ...
    <section class="section">
      <h3 class="section__title">
        3. Доступные заметки для выбранного пользователя
      </h3>
      ...
      <div v-for="n in notes" :key="n.id" class="note">
        ...
        <div class="note__meta">
          <span
            >owner: <code>{{ shortId(n.owner_user_id) }}</code></span
          >
        ...
        <div
          class="note__actions"
          v-if="userId && String(n.owner_user_id) === String(userId)"
        >
          <button
